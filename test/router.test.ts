...
describe('Router', () => {
  const token0 = new Token(ChainId.KCC, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.KCC, '0x0000000000000000000000000000000000000002', 18, 't1')

  const pair_0_1 = new Pair(new TokenAmount(token0, JSBI.BigInt(1000)), new TokenAmount(token1, JSBI.BigInt(1000)))

  const pair_weth_0 = new Pair(new TokenAmount(WETH[ChainId.KCC], '1000'), new TokenAmount(token0, '1000'))
    ...
    describe('exact in', () => {
      it('ether to token1', () => {
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WETH[ChainId.KCC].address, token0.address, token1.address],
      ...
      it('token1 to ether', () => {
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WETH[ChainId.KCC].address],
          '0x0000000000000000000000000000000000000004'
        ])
